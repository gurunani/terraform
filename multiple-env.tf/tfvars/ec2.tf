...
# }
resource "aws_instance" "roboshop" {
  count                  = length(keys(var.instances)) # use keys() to count
  ami                    = var.ami_id
  instance_type          = var.instance_type
  vpc_security_group_ids = [aws_security_group.allow_all.id]
  ...
  tags = merge(
    var.common_tags,
    {
      Name       = "${var.project}-${keys(var.instances)[count.index]}-${var.environment}"
      Component  = keys(var.instances)[count.index]
      Environment = var.environment
    }
  )
}
